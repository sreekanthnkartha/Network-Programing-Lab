...
int main()
{
    ...
    address.sin_port = htons(8887);                   // sin_port - port number in network byte order (htons - convert short integer to network byte order)
    // CREATE CONNECT
    ...
    while (1)
    {
        ...
        {                      // for - loop through the file descriptors.
            ...
            { // FD_ISSET - check if a file descriptor is set in a set.
                ...
                {
                    int len = recv(sock_fd, buffer, 1024, 0); // recv - receive data from a socket.
                    buffer[len] = '\0';                       // buffer[len] - end of string.
                    printf("%s\n", buffer);
                }
            }
        }
